...
void X10Firecracker::init( int _RTS_Pin, int _DTR_Pin )
{
	RTS_pin = _RTS_Pin;
	DTR_pin = _DTR_Pin;
...
void X10Firecracker::sendCmd(HouseCode house, int device, CommandCode cmnd)
{
	...
	delay(500);                   // need extra time for it to settle
	...
	delay(500);                         	 // wait required before next xmit
	// Pull pins low after xmit
